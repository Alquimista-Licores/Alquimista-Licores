-- Migration: Set pago_at when status is 'pago' in create_site_order_transactional
-- Also ensures manual orders created with 'pago' status get the timestamp.
...
BEGIN
    ...
    -- Se o status inicial for pago, define o timestamp
    IF p_status_inicial = 'pago' THEN
        v_pago_at := now();
    END IF;
    ...
    WHILE v_attempts < 10 LOOP
        BEGIN
            ...
            INSERT INTO public.site_orders (
                ...
                tipo_entrega, endereco_completo, status, pago_at
            ) VALUES (
                ...
                p_tipo_entrega, p_endereco_completo, p_status_inicial, v_pago_at
            )
