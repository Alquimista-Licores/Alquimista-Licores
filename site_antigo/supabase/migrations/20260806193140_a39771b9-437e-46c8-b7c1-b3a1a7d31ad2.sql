-- 1. Remover trigger e função se existirem (limpeza para garantir aplicação limpa)
...
-- 2. Remover função transacional se existir
...
BEGIN
    -- Idempotência: Verificar se o request_id já foi processado anteriormente
    ...
    IF FOUND THEN
        RETURN QUERY 
        SELECT o.id, o.codigo_pedido, o.status::text, o.created_at 
        FROM public.site_orders o 
        WHERE o.id = v_existing_id;
        RETURN;
    END IF;

    -- Loop para tratar colisão de código aleatório (máx 10 tentativas)
