...
- **Componente Dedicado ([`src/components/CustomCursor.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/components/CustomCursor.astro)) integrado globalmente em [`src/layouts/Layout.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/layouts/Layout.astro):**
  ...
  5. **Cards de Produto (Ocultação Instantânea do Círculo / 0% Opacidade):** No momento em que o ponto central entra em hover sobre um card de produto (`.product-card`), o círculo externo fica **instantaneamente com transparência 0%** (`opacity: 0 !important`), permitindo que apenas o ponto central permaneça visível acompanhando o mouse e que o efeito nativo de paralaxe 3D e o brilho (*glare*) do card brilhem com total clareza visual sem bordas externas concorrentes. Ao sair do card, o círculo reaparece imediatamente. Caso o mouse paire sobre o botão de caldeirão (`.quick-add-btn`) presente no card, este botão específico é englobado perfeitamente em formato circular.
