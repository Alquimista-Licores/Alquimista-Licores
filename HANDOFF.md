...
- **Componente Dedicado ([`src/components/CustomCursor.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/components/CustomCursor.astro)) integrado globalmente em [`src/layouts/Layout.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/layouts/Layout.astro):**
  ...
  3. **Botões e Cards (Contorno Transparente Flush Fit):** Ao pairar sobre botões, cards de produto (`.product-card`), botões de sugestão de poções (`.potion-suggestion-link` nos rituais), abas de filtro e inputs, o anel externo engloba o elemento **exatamente rente à borda** (0 padding) com **fundo 100% transparente** (sem alterar a cor do botão ou card), exibindo apenas a borda `#B28C46` de 1px com suave brilho áureo externo.
