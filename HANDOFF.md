...
- **`CustomCursor.astro`**: Cursor customizado de alta precisão em `#B28C46` com ponto central sem latência, englobamento magnético de 100px para botões e inputs, efeito de contorno circular (9999px) para botões de fechar modais (`✕`), quantidade (`+`, `-`), WhatsApp e Caldeirão, e isolamento de escopo quando modais ou a aba lateral do carrinho estão abertos.
