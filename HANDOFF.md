...
- **Componente Dedicado ([`src/components/CustomCursor.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/components/CustomCursor.astro)) integrado globalmente em [`src/layouts/Layout.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/layouts/Layout.astro):**
  ...
  5. **Botões e Cards (Contorno Transparente Flush Fit & Sincronização 3D Subpixel):** Ao englobar botões, cards de produto (`.product-card`), botões de poções (`.potion-suggestion-link`) e abas de filtro, o anel engloba **exatamente rente à borda** (0 padding) com **fundo 100% transparente**. Nos cards com efeito de tilt/paralaxe 3D, a moldura do cursor utiliza o centro de layout não-transformado via `DOMMatrix` e aplica a ordem de transformação CSS correta (`translate3d(origem) -> perspective/rotateX/rotateY/translateY -> translate3d(-50%, -50%)`), garantindo que o pivô de rotação e a inclinação 3D coincidam perfeitamente com os 4 cantos do card em tempo real, sem defasagem ou drift. Em hover direto sobre o card, o tracking de posição é instantâneo (zero lag), eliminando qualquer atraso em relação ao movimento físico do mouse.
