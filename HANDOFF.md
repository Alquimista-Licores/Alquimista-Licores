...
- **Componente Dedicado ([`src/components/CustomCursor.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/components/CustomCursor.astro)) integrado globalmente em [`src/layouts/Layout.astro`](file:///d:/projetos%20antigravity/site_alquimista/src/layouts/Layout.astro):**
  ...
  5. **Cards de Produto (Ocultação Instantânea do Círculo, Borda #B28C46 & Leve Brilho):** No momento em que o ponto central entra em hover sobre um card de produto (`.product-card`), o círculo externo fica **instantaneamente com transparência 0%** (`opacity: 0 !important`), enquanto a própria borda do card transiciona para a cor dourada do cursor (`#B28C46`) acompanhada de um **leve brilho dourado** (`box-shadow: ... 0 0 18px rgba(178, 140, 70, 0.42)`), sincronizado com o efeito de paralaxe 3D e o reflexo dinâmico (*glare*). Ao sair do card, a borda e o brilho retornam ao estado de repouso e o círculo externo do cursor reaparece. Caso o mouse paire sobre o botão de caldeirão (`.quick-add-btn`) presente no card, este botão específico é englobado perfeitamente em formato circular.
